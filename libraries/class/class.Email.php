...
class Email
{
    ...
    private function info()
    {
        ...
        $social = array();
        $socialString = '';
        $this->company = $this->d->rawQueryOne("select options, namevi from #_setting where id_shop = 0 limit 0,1");
        $this->optcompany = json_decode($this->company['options'], true);
        $logo = $this->d->rawQueryOne("select photo from #_photo where id_shop = 0 and type = ? and act = ? limit 0,1", array('logo', 'photo_static'));
        $social = $this->d->rawQuery("select photo, link from #_photo where id_shop = 0 and type = ? and find_in_set('hienthi',status) order by numb,id desc", array('social'));
