...
if (empty($id)) {
    ...
    if (in_array($sector['type'], array($config['website']['sectors']))) {
        ...
        /* Sector sub detail */
        if (!empty($IDSub)) {
            $sectorSubDetail = $cache->get("select name$lang, id, options, photo,properties from #_product_sub where id = ? and find_in_set('hienthi',status) limit 0,1", array($IDSub), 'fetch', 7200);
            /* Check sector */
    ...
    if (!empty($sector['tables'])) {
        ...
        if ($rowDetail['id_sub']>0) {

            $list_opt = $func -> getInfo('properties', 'product_sub', $rowDetail['id_sub']);
        ...
        /* Content */
        $rowDetailContent = $cache->get("select content$lang,properties$lang from #_$tableProductContent where id_parent = ? limit 0,1", array($rowDetail['id']), 'fetch', 7200);
        ...
        if (!empty($rowDetail['id_shop'])) {
            ...
            /* Get info of Shop */
            $sampleData = $cache->get("select id_interface,logo from #_sample where id_interface = ?", array($shopDetail['id_interface']), 'fetch', 7200);
            ...
            if (!empty($rowDetail['id_member'])) {
                ...
                /* Contact */
                $rowDetailContact = $cache->get("select id,fullname, phone, address, email from #_$tableProductContact where id_parent = ?", array($rowDetail['id']), 'fetch', 7200);
        ...
        /* SEO */
        $seoDB = (empty($videoDetail)) ? $seo->getOnDB('*', $tableProductSeo, $rowDetail['id']) : array();
        $seo->set('h1', $dataDetail['name' . $lang]);
        if (!empty($seoDB['title' . $seolang])) $seo->set('title', $seoDB['title' . $seolang]);
        else $seo->set('title', $dataDetail['name' . $lang]);
        if (!empty($seoDB['keywords' . $seolang])) $seo->set('keywords', $seoDB['keywords' . $seolang]);
        if (!empty($seoDB['description' . $seolang])) $seo->set('description', $seoDB['description' . $seolang]);
        $seo->set('url', $func->getPageURL());
        $img_json_bar = (!empty($dataDetail['options'])) ? json_decode($dataDetail['options'], true) : null;
        if (empty($img_json_bar) || ($img_json_bar['p'] != $dataDetail['photo'])) {
            $img_json_bar = $func->getImgSize($dataDetail['photo'], $dataDetail['upload'] . $dataDetail['photo']);
            $seo->updateSeoDB(json_encode($img_json_bar), $dataDetail['table'], $dataDetail['id']);
        }
        if (!empty($img_json_bar)) {
            $seo->set('photo', $configBase . THUMBS . '/' . $img_json_bar['w'] . 'x' . $img_json_bar['h'] . 'x2/' . $dataDetail['upload'] . $dataDetail['photo']);
            $seo->set('photo:width', $img_json_bar['w']);
            $seo->set('photo:height', $img_json_bar['h']);
            $seo->set('photo:type', $img_json_bar['m']);
        }
