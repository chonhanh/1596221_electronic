...
/* Lấy bài viết tĩnh */
$static = $d->rawQueryOne("select A.id as id, A.name$lang as name$lang, A.type as type, A.photo as photo, A.date_created as date_created, A.date_updated as date_updated, A.options as options, B.content$lang as content$lang from #_static as A, #_static_content as B where A.id_shop = $idShop and B.id_parent = A.id and A.type = ? and find_in_set('hienthi',A.status) limit 0,1", array($type));
...
if (!empty($static)) {
    ...
    if (!empty($img_json_bar)) {
        $seo->set('photo', $configBase . THUMBS . '/' . $img_json_bar['w'] . 'x' . $img_json_bar['h'] . 'x2/' . UPLOAD_NEWS_L . $static['photo']);
