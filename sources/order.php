...
if (!empty($_POST['submit-cart'])) {
    ...
    /* Data order info */
    $dataOrderInfo = array();
    $dataOrderInfo['id_order'] = $idNewOrder;
    $dataOrderInfo['id_region'] = $region;
    $dataOrderInfo['id_city'] = $city;
    $dataOrderInfo['id_district'] = $district;
    $dataOrderInfo['id_wards'] = $wards;
    $dataOrderInfo['fullname'] = $fullname;
    $dataOrderInfo['phone'] = $phone;
    $dataOrderInfo['address'] = $address;
    $dataOrderInfo['email'] = $email;

    /* Save order main */
    if (!$d->insert('order_info', $dataOrderInfo)) {
        /* Delete data order main */
        $d->rawQuery("delete from #_order where id = ?", array($idNewOrder));

        /* Transfer */
        $func->transfer("Xử lý đơn hàng có vấn đề. Vui lòng thử lại sau.", $configBase . 'gio-hang', false);
    } else {
        $idNewOrderInfo = $d->getLastInsertId();
    }
    ...
    foreach ($_SESSION['owner']['cart'] as $k_group_cart => $v_group_cart) {
        ...
        if (!$d->insert('order_group', $dataOrderGroup)) {
            /* Delete data order main */
            $d->rawQuery("delete from #_order_info where id = ?", array($idNewOrderInfo));

            /* Transfer */
            $func->transfer("Xử lý đơn hàng có vấn đề. Vui lòng thử lại sau.", $configBase . 'gio-hang', false);
        } else {
            $idNewOrderGroup = $d->getLastInsertId();
        }
        ...
        foreach ($v_group_cart['lists'] as $v_cart) {
            ...
            $color = $cart->getSale($v_cart['color'], 'color');
            $size = $cart->getSale($v_cart['size'], 'size');

            /* Data order detail */
            $dataOrderDetail = array();
            $dataOrderDetail['id_order'] = $idNewOrder;
            $dataOrderDetail['id_group'] = $idNewOrderGroup;
            ...
            $dataOrderDetail['sector_prefix'] = $v_cart['prefix'];

            /* Save order detail */
            $d->insert('order_detail', $dataOrderDetail);
        }
