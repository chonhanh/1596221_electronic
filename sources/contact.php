...
if (!empty($_POST['submit-contact'])) {
    ...
    $sessionCaptcha = (!empty($_SESSION["captcha"]["contact"])) ? $_SESSION["captcha"]["contact"] : null;
    ...
    if (($scoreCaptcha >= 0.5 && $actionCaptcha == 'Contact') || $testCaptcha == true) {
        ...
        if (!empty($response)) {
            ...
            unset($_SESSION["captcha"]["contact"]);
        ...
        if ($d->insert('contact', $data)) {
            ...
            if ($func->hasFile("file_attach")) {
                ...
                if ($file_attach = $func->uploadImage("file_attach", '.doc|.docx|.pdf|.rar|.zip|.ppt|.pptx|.xls|.xlsx|.jpg|.png|.gif', UPLOAD_FILE_L, $file_name)) {
...
/* Lấy dữ liệu */
$contact = $d->rawQueryOne("select A.id as id, A.name$lang as name$lang, A.photo as photo, A.options as options, B.content$lang as content$lang from #_static as A, #_static_content as B where A.id_shop = $idShop and B.id_parent = A.id and A.type = ? limit 0,1", array('lien-he'));
...
if (empty($img_json_bar) || ($img_json_bar['p'] != $contact['photo'])) {
    $img_json_bar = $func->getImgSize($contact['photo'], UPLOAD_NEWS_L . $contact['photo']);
    $seo->updateSeoDB(json_encode($img_json_bar), 'static', $contact['id']);
}
if (!empty($img_json_bar)) {
    $seo->set('photo', $configBase . THUMBS . '/' . $img_json_bar['w'] . 'x' . $img_json_bar['h'] . 'x2/' . UPLOAD_NEWS_L . $contact['photo']);
