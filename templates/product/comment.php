<div class="comment-page p-3">
    <!-- Statistic comment -->
    <div class="comment-statistic">
        <div class="card border-0">
            <div class="card-header text-uppercase border-0"><strong>Bình luận sản phẩm</strong></div>
                ...
                <div class="row align-items-center py-3">
                    ...
                    <div class="col-sm-12 col-lg-4">
                        <div class="text-center">
                            ...
                            <button type="button" class="btn btn-sm btn-warning btn-write-comment font-weight-500 py-2 px-3">Viết nhận xét của bạn</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form id="form-comment" action="" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group review-preview">
                                    <div class="row">
                                        ...
                                        <div class="col-8">
                                            ...
                                            <div class="text-split mb-0"><?= $func->decodeHtmlChars($rowDetail['rowDetailContent']['content' . $lang]) ?></div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <button type="submit" class="btn btn-sm btn-warning font-weight-500 text-capitalize py-2 px-3">Gửi nhận xét</button>
                        <input type="hidden" name="dataReview[id_shop]" value="<?= $rowDetail['id_shop'] ?>">
                        <input type="hidden" name="dataReview[id_product]" value="<?= $rowDetail['id'] ?>">
                        <input type="hidden" name="dataReview[variant]" value="<?= (!empty($rowDetail['id_shop'])) ? 'shop' : 'personal' ?>">
                    </form>
                </div>
            </div>
        </div>
                ...
                <div class="card-body pt-5 pb-3">
                    <div class="comment-load">
                        ...
                        foreach ($comment->lists as $v_lists) {
                            ...
                            $comment->params['id_product'] = $rowDetail['id'];
                            $comment->params['is_owner'] = ($isLogin && (($idMember == $rowDetail['id_member']) || (!empty($rowDetail['rowDetailShop']) && $idMember == $rowDetail['rowDetailShop']['id_member']))) ? true : false;
                            $comment->params['variant'] = (!empty($rowDetail['id_shop'])) ? 'shop' : 'personal';
                            ...
                            $comment->params['lists']['replies'] = $comment->replies($v_lists['id'], $rowDetail['id']);
                    ...
                    <?php if ($comment->total > $comment->limitParentShow) { ?>
                        <div class="comment-load-more-control text-center mt-4">
                            ...
                            <input type="hidden" class="id-product" value="<?= $rowDetail['id'] ?>">
                            <input type="hidden" class="is-owner" value="<?= ($isLogin && (($idMember == $rowDetail['id_member']) || (!empty($rowDetail['rowDetailShop']) && $idMember == $rowDetail['rowDetailShop']['id_member']))) ? '1' : '0' ?>">
                            <input type="hidden" class="variant" value="<?= (!empty($rowDetail['id_shop'])) ? 'shop' : 'personal' ?>">
