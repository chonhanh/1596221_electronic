...
<div class="content-main">
            ...
            <?php foreach ($sectors as $v_sector) { ?>
                <div class="col-4 mb-3">
                    <div class="sectors sectors-list">
                        <a class="sectors-image scale-img" href="tim-kiem?sector=<?= $v_sector['id'] ?>&keyword=<?= $keyword ?>" title="<?= $v_sector['name' . $lang] ?>">
                        ...
                        <h3 class="sectors-name d-flex align-items-center justify-content-center w-100 mt-3 mb-0 px-2 text-center">
                            <a class="text-decoration-none text-primary-hover text-uppercase transition" href="tim-kiem?sector=<?= $v_sector['id'] ?>&keyword=<?= $keyword ?>" title="<?= $v_sector['name' . $lang] ?>"><?= $v_sector['name' . $lang] ?></a>
                        </h3>
                    </div>
                </div>
            <?php } ?>
    ...
    <?php } else { ?>
        <div class="lists-sector mb-4">
            <?php /*foreach ($sectors as $v_sector) { ?>
                <a class="btn btn-sm <?= ($IDSector == $v_sector['id']) ? 'btn-primary' : 'btn-outline-primary' ?> d-inline-block text-capitalize py-2 px-3 mr-2" href="tim-kiem?sector=<?= $v_sector['id'] ?>&keyword=<?= $keyword ?>" title="<?= $v_sector['name' . $lang] ?>"><?= $v_sector['name' . $lang] ?></a>
            <?php }*/ ?>
        </div>
