...
if ($cmd == 'add-cart' && $id > 0) {
    ...
    if (!empty($_SESSION['owner']['cart'])) {
        foreach ($_SESSION['owner']['cart'][$groupCode]['lists'] as $k_cart => $v_cart) {
            if ($code == $v_cart['code']) {
                if ($quantity) {
                    $infoProduct = $_SESSION['owner']['cart'][$groupCode]['lists'][$k_cart];
                    $_SESSION['owner']['cart'][$groupCode]['lists'][$k_cart]['qty'] = $quantity;
                }
    ...
    $max = (!empty($_SESSION['owner']['cart'])) ? count($_SESSION['owner']['cart']) : 0;
    $total = $cart->getOrderTotal();
    $totalText = $func->formatMoney($total);
