...
if (!empty($id)) {
    /* Lấy bài viết detail */
    $rowDetail = $d->rawQueryOne("select A.id as id, A.view as view, A.date_created as date_created, A.type as type, A.name$lang as name$lang, A.slugvi as slugvi, A.slugen as slugen, A.photo as photo, A.options as options, B.desc$lang as desc$lang, B.content$lang as content$lang from #_news as A, #_news_content as B where A.id_shop = $idShop and A.sector_prefix = ? and B.id_parent = A.id and A.id = ? and A.type = ? and find_in_set('hienthi',A.status) limit 0,1", array($prefixSector, $id, $type));
    ...
    $where = "";
    $where = "A.id_shop = $idShop and A.sector_prefix = ? and A.id <> ? and A.type = ? and find_in_set('hienthi',A.status)";
    $params = array($prefixSector, $id, $type);
    ...
    $sqlNum = "select count(*) as 'num' from #_news as A where $where order by A.numb,A.id desc";
    ...
    if (empty($img_json_bar) || ($img_json_bar['p'] != $rowDetail['photo'])) {
        $img_json_bar = $func->getImgSize($rowDetail['photo'], UPLOAD_NEWS_SOURCE . $rowDetail['photo']);
        $seo->updateSeoDB(json_encode($img_json_bar), 'news', $rowDetail['id']);
    }
    if (!empty($img_json_bar)) {
        $seo->set('photo', $configBase . THUMBS_SOURCE . '/' . $img_json_bar['w'] . 'x' . $img_json_bar['h'] . 'x2/' . UPLOAD_NEWS_THUMB . $rowDetail['photo']);
    ...
    if (!empty($seopage['photo'])) {
        if (empty($img_json_bar) || ($img_json_bar['p'] != $seopage['photo'])) {
            $img_json_bar = $func->getImgSize($seopage['photo'], UPLOAD_SEOPAGE_SOURCE . $seopage['photo']);
            $seo->updateSeoDB(json_encode($img_json_bar), 'seopage', $seopage['id']);
        }
        if (!empty($img_json_bar)) {
            $seo->set('photo', $configBase . THUMBS_SOURCE . '/' . $img_json_bar['w'] . 'x' . $img_json_bar['h'] . 'x2/' . UPLOAD_SEOPAGE_THUMB . $seopage['photo']);
            $seo->set('photo:width', $img_json_bar['w']);
            $seo->set('photo:height', $img_json_bar['h']);
            $seo->set('photo:type', $img_json_bar['m']);
        }
    ...
    /* Lấy tất cả bài viết */
    $where = "";
    $where = "A.id_shop = $idShop and A.sector_prefix = ? and B.id_parent = A.id and A.type = ? and find_in_set('hienthi',A.status)";
    $params = array($prefixSector, $type);
