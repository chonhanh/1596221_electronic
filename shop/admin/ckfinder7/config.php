...
$config['authentication'] = function () {
    $token = (!empty($_GET['token'])) ? $_GET['token'] : '';
    return (!empty($token) && !empty($_SESSION[$token]['active'])) ? true : false;
};
...
$config['backends'][] = array(
    'name'         => 'default',
    'adapter'      => 'local',
    'baseUrl'      => str_replace('shop/admin/ckfinder7/core/connector/php/', '', baseUrl()).'upload/filemanager/'.$_SESSION[$token]['folder'].'/',
    'root'         => dirname(dirname(dirname(__DIR__))).'/upload/filemanager/'.$_SESSION[$token]['folder'].'/',
    'chmodFiles'   => 0777,
    'chmodFolders' => 0755,
