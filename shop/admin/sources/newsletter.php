...
if (isset($_POST["listemail"]) && isset($_POST['subject']) && isset($_POST['content'])) {
    ...
    for ($i = 0; $i < count($listemail); $i++) {
        ...
        $row = $d->rawQueryOne("select id, email, fullname from #_newsletter where id_shop = $idShop and sector_prefix = ? and id = ? and type = ? limit 0,1", array($prefixSector, $id, $type));
...
function viewMans()
{
    ...
    $sql = "select * from #_newsletter where id_shop = $idShop and sector_prefix = ? and type = ? $where order by numb,id desc $limit";
    $items = $d->rawQuery($sql, array($prefixSector, $type));
    $sqlNum = "select count(*) as 'num' from #_newsletter where id_shop = $idShop and sector_prefix = ? and type = ? $where order by numb,id desc";
...
function editMan()
{
    global $d, $idShop, $prefixSector, $func, $curPage, $item, $type;

    $id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

    if (empty($id)) $func->transfer("Không nhận được dữ liệu", "index.php?com=newsletter&act=man&type=" . $type . "&p=" . $curPage, false);

    $item = $d->rawQueryOne("select * from #_newsletter where id_shop = $idShop and sector_prefix = ? and id = ? and type = ? limit 0,1", array($prefixSector, $id, $type));
...
function deleteMan()
{
    ...
    if ($id) {
        $row = $d->rawQueryOne("select id from #_newsletter where id_shop = $idShop and sector_prefix = ? and id = ? and type = ? limit 0,1", array($prefixSector, $id, $type));

        if (isset($row['id']) && $row['id'] > 0) {
            $d->rawQuery("delete from #_newsletter where id_shop = $idShop and sector_prefix = ? and id = ? and type = ?", array($prefixSector, $id, $type));
