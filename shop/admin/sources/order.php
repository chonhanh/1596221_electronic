...
function viewMans()
{
    ...
    if ($order_status) $where .= " and A.order_status=$order_status";
    if ($order_payment) $where .= " and O.order_payment=$order_payment";
    if ($order_date) {
    ...
    if ($range_price) {
        ...
        $where .= " and A.total_price<=$price_to and A.total_price>=$price_from";
    }
    if ($city) $where .= " and B.id_city=$city";
    if ($district) $where .= " and B.id_district=$district";
    if ($wards) $where .= " and B.id_wards=$wards";
    if (isset($_REQUEST['keyword'])) {
        $keyword = htmlspecialchars($_REQUEST['keyword']);
        $where .= " and (O.code LIKE '%$keyword%' or B.fullname LIKE '%$keyword%' or B.email LIKE '%$keyword%' or B.phone LIKE '%$keyword%')";
    }

    $perPage = 10;
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select A.*, O.code as code, O.order_payment as order_payment, B.fullname as fullname from #_order_group as A, #_order as O, #_order_info as B where A.id_shop = $idShop and A.sector_prefix = ? and A.id_order = O.id and A.id_order = B.id_order $where order by A.date_created desc $limit";
    $items = $d->rawQuery($sql, array($prefixSector));
    $sqlNum = "select count(*) as 'num' from #_order_group as A, #_order as O, #_order_info as B where A.id_shop = $idShop and A.sector_prefix = ? and A.id_order = O.id and A.id_order = B.id_order $where order by A.date_created desc";
    ...
    /* Lấy tổng giá min */
    $minTotal = $d->rawQueryOne("select min(total_price) from #_order_group where id_shop = $idShop and sector_prefix = ?", array($prefixSector));
...
function editMan()
{
    ...
    if (empty($id)) {
        ...
        $item = $d->rawQueryOne("select A.*, O.code as code, O.order_payment as order_payment, B.fullname as fullname, B.phone as phone, B.email as email, B.address as address from #_order_group as A, #_order as O, #_order_info as B where A.id_order = O.id and A.id_order = B.id_order and A.id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer("Dữ liệu không có thực", "index.php?com=order&act=man&p=" . $curPage, false);
        } else {
            /* Get order detail */
            $orderDetails = $d->rawQuery("select * from #_order_detail where id_order = ? and id_group = ?", array($item['id_order'], $id));
        }
    }
}
