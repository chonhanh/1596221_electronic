...
function viewMans()
{
    global $d, $idShop, $prefixSector, $func, $strUrl, $curPage, $items, $paging, $type;
    ...
    $sql = "select * from #_news where id_shop = $idShop and sector_prefix = ? and type = ? $where order by numb,id desc $limit";
    $items = $d->rawQuery($sql, array($prefixSector, $type));
    $sqlNum = "select count(*) as 'num' from #_news where id_shop = $idShop and sector_prefix = ? and type = ? $where order by numb,id desc";
    $count = $d->rawQueryOne($sqlNum, array($prefixSector, $type));
...
function editMan()
{
    global $d, $idShop, $prefixSector, $strUrl, $func, $curPage, $item, $itemContent, $type, $com, $act;

    $id = (!empty($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

    if (empty($id)) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);

    $item = $d->rawQueryOne("select * from #_news where id_shop = $idShop and sector_prefix = ? and id = ? and type = ? limit 0,1", array($prefixSector, $id, $type));
...
function saveMan()
{
    global $d, $idShop, $prefixSector, $strUrl, $func, $flash, $curPage, $config, $com, $act, $type;
        ...
        if ($d->update('news', $data)) {
            ...
            if ($func->hasFile("file")) {
                ...
                if ($photo = $func->uploadImage("file", $config['news'][$type]['img_type'], UPLOAD_NEWS, $file_name)) {
                    $row = $d->rawQueryOne("select id, photo from #_news where id_shop = $idShop and sector_prefix = ? and id = ? and type = ? limit 0,1", array($prefixSector, $id, $type));
...
function deleteMan()
{
    global $d, $idShop, $prefixSector, $strUrl, $func, $curPage, $com, $type;
    ...
    if ($id) {
        ...
        if (isset($row['id']) && $row['id'] > 0) {
            /* Xóa chính */
            $func->deleteFile(UPLOAD_NEWS . $row['photo']);
            $d->rawQuery("delete from #_news where id_shop = $idShop and sector_prefix = ? and id = ?", array($prefixSector, $id));
