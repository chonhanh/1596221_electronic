<?php
$linkMan = $linkFilter = "index.php?com=product&act=man_sub";
$linkAdd = "index.php?com=product&act=add_sub";
$linkEdit = "index.php?com=product&act=edit_sub";
$linkDelete = "index.php?com=product&act=delete_sub";
?>
<!-- Content Header -->
    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active"><?= $config['product']['title_main_sub'] ?></li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    ...
    <div class="card-footer form-group-category text-sm bg-light row">
        ...
        <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2">
            <label class="d-block"><?=$config['product']['title_main_item']?></label>
            <?= $func->getLinkCategory('product', 'item') ?>
        </div>
    </div>
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title"><?= $config['product']['title_main_sub'] ?></h3>
        </div>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="align-middle" style="width:80%">Tiêu đề</th>
                        <?php if (isset($config['product']['check_shop_sub'])) {
                            foreach ($config['product']['check_shop_sub'] as $key => $value) { ?>
                        ...
                        <?php for ($i = 0; $i < count($items); $i++) {
                            ...
                            <tr>
                                <td class="align-middle">
                                    <img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?= THUMBS ?>/<?= $config['product']['thumb_sub'] ?>/<?= UPLOAD_PRODUCT_THUMB . $items[$i]['photo'] ?>" alt="<?= $items[$i]['namevi'] ?>">
                                </td>
                                <td class="align-middle">
                                    ...
                                    foreach ($config['product']['check_shop_sub'] as $key => $value) { ?>
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox">
                                                <input type="checkbox" class="custom-control-input show-checkbox-shop" id="show-checkbox-<?= $key ?>-<?= $items[$i]['id'] ?>" data-table="product_sub_status" data-id="<?= $items[$i]['id'] ?>" data-attr="<?= $key ?>" <?= (in_array($key, $status_array)) ? 'checked' : '' ?>>
