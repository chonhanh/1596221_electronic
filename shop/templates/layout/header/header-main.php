<div class="header" id="header">
    <div class="block-content d-flex align-items-center justify-content-between">
        ...
        <a class="header-logo text-decoration-none" href="" title="<?= $setting['name' . $lang] ?>">
            <?= $func->getImage(['sizes' => '240x0x3', 'upload' => UPLOAD_PHOTO_L, 'image' => $logo_main['photo'], 'alt' => $setting['name' . $lang]]) ?>
        </a>
        ...
        <a class="sectors-by-list header-info nav-icon text-decoration-none" href="javascript:void(0)" data-plugin="tooltip" data-id="<?=$shopInfo['id_list']?>" data-placement="bottom" title="Danh mục chính">
            <?= $func->getImage(['class' => 'lazy mr-2', 'size-error' => '30x25x2', 'upload' => 'assets/images/', 'image' => 'ic_bar.png', 'alt' => 'Danh mục thời trang']) ?>
            <span class="text-uppercase">Danh mục<br>Thời trang</span>
        </a>
        ...
        <?php if (!empty($func->getMember('active'))) { ?>
            <a class="header-account-dashboard header-info nav-icon text-decoration-none" href="account/dashboard" data-plugin="tooltip" data-placement="bottom" title="Trang cá nhân">
                <?= $func->getImage(['class' => 'lazy rounded-circle mr-2', 'sizes' => '40x40x1', 'upload' => UPLOAD_USER_L, 'image' => $func->getMember('avatar'), 'alt' => 'Trang cá nhân']) ?>
