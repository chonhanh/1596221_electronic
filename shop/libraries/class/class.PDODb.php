...
class PDODb
{
    ...
    private function buildInsert($tableName, $insertData, $operation)
    {
        ...
        $this->createLogs(@$_SESSION[$loginShop]['shop']['email'], 'insert', $tableName, $this->pdo()
            ->lastInsertId());
        $this->reset();
        if (
            $status && $this->pdo()
    ...
    public function rawQuery($query, $params = null)
    {
        ...
        if (strpos($this->query, 'delete') !== false) {
            ...
            $this->createLogs(@$_SESSION[$loginShop]['shop']['email'], 'query', "tables", $queryLogs);
        }
        $this->reset();
        return $result;
    }
    ...
    public function update($tableName, $tableData, $numRows = null)
    {
        ...
        $this->createLogs(@$_SESSION[$loginShop]['shop']['email'], 'update', $tableName, $wherelogs);
        $this->reset();
        $this->rowCount = $stmt->rowCount();
        return $status;
    }
    ...
    /* Maintenance */
    public function runMaintain()
    {
        $tables = $this->rawQuery("SHOW TABLES");
        $actions = array('REPAIR', 'OPTIMIZE');

        if (!empty($tables)) {
            foreach ($actions as $v_action) {
                foreach ($tables as $k_table => $v_table) {
                    foreach ($v_table as $table) {
                        $this->rawQuery("$v_action TABLE $table");
                    }
                }
            }
        }
