...
/* Array folders */
$upload_const = 'upload';
$array_const = array('file', 'filemanager', 'sync', 'excel', 'seopage', 'photo', 'temp', 'user', 'product', 'store', 'shop', 'video', 'color', 'interface', 'news', 'tags');
...
if (file_exists(ROOT . $upload_const) && $array_const) {
    ...
    foreach ($array_const as $vconst) {
        ...
        $define_in_admin = '../../../' . $upload_const . '/' . $define_lower_const . '/';
        $define_out = '../../' . $upload_const . '/' . $define_lower_const . '/';
        $define_source = '../' . $upload_const . '/' . $define_lower_const . '/';
        $define_thumb = $upload_const . '/' . $define_lower_const . '/';
        if (!defined($define_upper_upload . '_' . $define_upper_const) && !defined($define_upper_upload . '_' . $define_upper_const . '_L')) {
            ...
            $define_constants[$define_upper_upload . '_' . $define_upper_const . '_ADMIN'] = $define_in_admin;
            $define_constants[$define_upper_upload . '_' . $define_upper_const . '_SOURCE'] = $define_source;
            $define_constants[$define_upper_upload . '_' . $define_upper_const . '_THUMB'] = $define_thumb;
    ...
    if (!empty($define_constants)) {
        $path_upload = ROOT . 'shop/' . basename(__DIR__) . '/upload.php';
