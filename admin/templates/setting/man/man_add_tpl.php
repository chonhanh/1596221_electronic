...
<section class="content">
    <form method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <?php if (isset($config['website']['debug-developer']) && $config['website']['debug-developer'] == true) { ?>
            ...
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
                    <h3 class="card-title">Tạo dữ liệu json</h3>
                </div>
                <div class="card-body">
                    <a class="btn btn-sm bg-gradient-success mr-1" href="<?= $linkJson ?>&type=list" title="Danh mục cấp 1">Danh mục cấp 1</a>
                    <a class="btn btn-sm bg-gradient-success mr-1" href="<?= $linkJson ?>&type=cat" title="Danh mục cấp 2">Danh mục cấp 2</a>
                    <a class="btn btn-sm bg-gradient-success mr-1" href="<?= $linkJson ?>&type=item" title="Danh mục cấp 3">Danh mục cấp 3</a>
                    <a class="btn btn-sm bg-gradient-success mr-1" href="<?= $linkJson ?>&type=store" title="Cửa hàng">Cửa hàng</a>
                    <a class="btn btn-sm bg-gradient-success mr-1" href="<?= $linkJson ?>&type=region" title="Vùng/miền">Vùng/miền</a>
                    <a class="btn btn-sm bg-gradient-success mr-1" href="<?= $linkJson ?>&type=city" title="Tỉnh/thành phố">Tỉnh/thành phố</a>
                    <a class="btn btn-sm bg-gradient-success mr-1" href="<?= $linkJson ?>&type=district" title="Quận/huyện">Quận/huyện</a>
                    <a class="btn btn-sm bg-gradient-success mr-1" href="<?= $linkJson ?>&type=wards" title="Phường/xã">Phường/xã</a>
                </div>
            </div>
        <?php } ?>
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control text-sm" name="data[analytics]" id="analytics" rows="5" placeholder="Google analytics"><?= $func->decodeHtmlChars($flash->get('coords_iframe')) ?: $func->decodeHtmlChars(@$options['coords_iframe']) ?></textarea>
                </div>
                <div class="form-group">
        ...
        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title">Thông tin nhãn tiêu đề</h3>
            </div>
            <div class="card-body">
                <div class="form-group">
                    <label for="greeting">Lời Chào:</label>
                    <input type="text" class="form-control text-sm" name="data[options][greeting]" id="greeting" placeholder="Tiêu đề Lời Chào" value="<?= (!empty($flash->has('greeting'))) ? $flash->get('greeting') : @$options['greeting'] ?>">
                </div>
                <div class="form-group">
                    <label for="group-store">Nhóm cửa hàng:</label>
                    <input type="text" class="form-control text-sm" name="data[options][group-store]" id="group-store" placeholder="Tiêu đề nhóm cửa hàng" value="<?= (!empty($flash->has('group-store'))) ? $flash->get('group-store') : @$options['group-store'] ?>">
                </div>
            </div>
        </div>
        <div class="card card-primary card-outline text-sm">
