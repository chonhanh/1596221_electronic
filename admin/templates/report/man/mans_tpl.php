<?php
$linkMan = "index.php?com=report&act=man";
$linkAdd = "index.php?com=report&act=add";
$linkEdit = "index.php?com=report&act=edit";
$linkDelete = "index.php?com=report&act=delete";
?>
<!-- Content Header -->
...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title">Danh sách tình trạng</h3>
        </div>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php if (isset($config['report']['check'])) {
                            foreach ($config['report']['check'] as $key => $value) { ?>
                    ...
                    <tbody>
                        <?php for ($i = 0; $i < count($items); $i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-mini m-auto update-numb" min="0" value="<?= $items[$i]['numb'] ?>" data-id="<?= $items[$i]['id'] ?>" data-table="report_status">
                                </td>
                                <td class="align-middle">
                                    <a class="text-dark text-break" href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['namevi'] ?>"><?= $items[$i]['namevi'] ?></a>
                                </td>
                                <td class="align-middle"><?= $items[$i]['descvi'] ?></td>
                                <?php $status_array = (!empty($items[$i]['status'])) ? explode(',', $items[$i]['status']) : array(); ?>
                                <?php if (isset($config['report']['check'])) {
                                    foreach ($config['report']['check'] as $key => $value) { ?>
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox">
                                                <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?= $key ?>-<?= $items[$i]['id'] ?>" data-table="report_status" data-id="<?= $items[$i]['id'] ?>" data-attr="<?= $key ?>" <?= (in_array($key, $status_array)) ? 'checked' : '' ?>>
