...
<section class="content">
    <form method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        <div class="card-footer text-sm sticky-top">
            <button type="submit" class="btn btn-sm bg-gradient-primary submit-newsletter"><i class="far fa-save mr-2"></i>Lưu</button>
            ...
            <div class="card-body">
                ...
                <?php if (isset($config['newsletter'][$type]['content']) && $config['newsletter'][$type]['content'] == true) { ?>
                    <div class="form-group">
                        ...
                        <textarea class="form-control text-sm" name="data[content]" id="content" rows="5" placeholder="Nội dung" required><?= $func->decodeHtmlChars($flash->get('content' . $k)) ?: $func->decodeHtmlChars(@$item['content' . $k]) ?></textarea>
                    </div>
                <?php } ?>
                <?php if (isset($config['newsletter'][$type]['notes']) && $config['newsletter'][$type]['notes'] == true) { ?>
                    <div class="form-group">
                        <label for="notes">Ghi chú:</label>
                        <textarea class="form-control text-sm" name="data[notes]" id="notes" rows="5" placeholder="Ghi chú"><?= $func->decodeHtmlChars($flash->get('notes' . $k)) ?: $func->decodeHtmlChars(@$item['notes' . $k]) ?></textarea>
                    </div>
                <?php } ?>
                <div class="form-group">
