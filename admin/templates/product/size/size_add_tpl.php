...
$linkSave = "index.php?com=product&act=save_size";
?>
<!-- Content Header -->
    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">Chi tiết kích cỡ</li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        <div class="card-footer text-sm sticky-top">
            <button type="submit" class="btn btn-sm bg-gradient-primary submit-size" disabled><i class="far fa-save mr-2"></i>Lưu</button>
            <a class="btn btn-sm bg-gradient-danger" href="<?= $linkMan ?>" title="Thoát"><i class="fas fa-sign-out-alt mr-2"></i>Thoát</a>
        </div>

        <?= $flash->getMessages('admin') ?>

        <div class="card-footer form-group-category text-sm bg-light row d-none">
            <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2">
                <?= $func->getAjaxCategory('product', 'list', true, true) ?>
            </div>
        </div>

        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title"><?= ($act == "edit_size") ? "Cập nhật" : "Thêm mới"; ?> kích cỡ</h3>
            </div>
            <div class="card-body">
