    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active"><?= $config['product']['title_main_cat'] ?></li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title"><?= $config['product']['title_main_cat'] ?></h3>
        </div>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="align-middle">Hình công ty</th>
                        <th class="align-middle" style="width:15%">Tiêu đề công ty</th>
                    ...
                    <tbody>
                        <?php for ($i = 0; $i < count($items); $i++) {
                            ...
                            <tr>
                                ...
                                <td class="align-middle">
                                    <a href="<?= $linkEdit ?><?= $linkID ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['namevi'] ?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?= THUMBS ?>/<?= $config['product']['thumb_cat'] ?>/<?= UPLOAD_PRODUCT_L . $items[$i]['photo'] ?>" alt="<?= $items[$i]['namevi'] ?>"></a>
                                </td>
                                <td class="align-middle">
                                    <a class="text-dark text-break" href="<?= $linkEdit ?><?= $linkID ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['namevi'] ?>"><?= $items[$i]['namevi'] ?></a>
                                </td>
                                <td class="align-middle">
                                    <a href="<?= $linkEdit ?><?= $linkID ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['name_storevi'] ?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?= THUMBS ?>/<?= $config['product']['thumb2_cat'] ?>/<?= UPLOAD_PRODUCT_L . $items[$i]['photo2'] ?>" alt="<?= $items[$i]['name_storevi'] ?>"></a>
                                </td>
                                <td class="align-middle">
                                    <a class="text-dark text-break" href="<?= $linkEdit ?><?= $linkID ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['name_storevi'] ?>"><?= $items[$i]['name_storevi'] ?></a>
                                </td>
