            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php if (isset($config['product']['check_color'])) {
                            foreach ($config['product']['check_color'] as $key => $value) { ?>
                    ...
                    <tbody>
                        <?php for ($i = 0; $i < count($items); $i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-mini m-auto update-numb" min="0" value="<?= $items[$i]['numb'] ?>" data-id="<?= $items[$i]['id'] ?>" data-table="product_color">
                                </td>
                                <?php if (isset($config['product']['color_images']) && $config['product']['color_images'] == true) { ?>
                                    <td class="align-middle">
                                        <a href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['namevi'] ?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?= THUMBS ?>/<?= $config['product']['thumb_color'] ?>/<?= UPLOAD_COLOR_L . $items[$i]['photo'] ?>" alt="<?= $items[$i]['namevi'] ?>"></a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
                                    <p class="mb-1"><a class="text-dark text-break" href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['namevi'] ?>"><?= $items[$i]['namevi'] ?></a></p>
                                ...
                                <?php if (isset($config['product']['check_color'])) {
                                    foreach ($config['product']['check_color'] as $key => $value) { ?>
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox">
                                                <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?= $key ?>-<?= $items[$i]['id'] ?>" data-table="product_color" data-id="<?= $items[$i]['id'] ?>" data-attr="<?= $key ?>" <?= (in_array($key, $status_array)) ? 'checked' : '' ?>>
