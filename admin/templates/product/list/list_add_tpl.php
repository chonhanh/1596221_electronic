...
$linkSave = "index.php?com=product&act=save_list";
?>
<!-- Content Header -->
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
                            ...
                            <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                    <div class="form-group">
                                        ...
                                        <?php if (empty($config['website']['debug-developer'])) { ?>
                                            <input type="text" class="form-control text-sm" id="name<?= $k ?>" placeholder="Tiêu đề (<?= $k ?>)" value="<?= @$item['name' . $k] ?>" readonly>
                                        <?php } else { ?>
                                            <input type="text" class="form-control for-seo text-sm" name="data[name<?= $k ?>]" id="name<?= $k ?>" placeholder="Tiêu đề (<?= $k ?>)" value="<?= @$item['name' . $k] ?>" required>
                                        <?php } ?>
                                    </div>
                                </div>
                            <?php } ?>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-4">
                        ...
                        $dimension = "Width: " . $config['product']['width_list'] . " px - Height: " . $config['product']['height_list'] . " px (" . $config['product']['img_type_list'] . ")";
                    ...
                    <div class="col-4">
                        <?php
                            ...
                            $dimension2 = "Width: ".$config['product']['width2_list']." px - Height: ".$config['product']['height2_list']." px (".$config['product']['img_type_list'].")";
                    ...
                    <div class="col-4">
                        ...
                        $dimension3 = "Width: " . $config['product']['width3_list'] . " px - Height: " . $config['product']['height3_list'] . " px (" . $config['product']['img_type_list'] . ")";
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                <?php
                $seoDB = $seo->getOnDB('*', 'product_list_seo', $id);
