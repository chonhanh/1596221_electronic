...
function viewMans()
{
    global $d, $idShop, $func, $curPage, $items, $paging;
    ...
    $sql = "select * from #_contact where id_shop = $idShop and id<>0 $where order by numb,id desc $limit";
    $items = $d->rawQuery($sql);
    $sqlNum = "select count(*) as 'num' from #_contact where id_shop = $idShop and id<>0 $where order by numb,id desc";
...
function editMan()
{
    global $d, $idShop, $func, $curPage, $item;

    $id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

    if (empty($id)) $func->transfer("Không nhận được dữ liệu", "index.php?com=contact&act=man&p=" . $curPage, false);

    $item = $d->rawQueryOne("select * from #_contact where id_shop = $idShop and id = ? limit 0,1", array($id));
...
function saveMan()
{
    global $d, $idShop, $func, $flash, $curPage;
...
function deleteMan()
{
    ...
    if ($id) {
        $row = $d->rawQueryOne("select id, file_attach from #_contact where id_shop = $idShop and id = ? limit 0,1", array($id));

        if (isset($row['id']) && $row['id'] > 0) {
            $func->deleteFile(UPLOAD_FILE . $row['file_attach']);
            $d->rawQuery("delete from #_contact where id_shop = $idShop and id = ?", array($id));
