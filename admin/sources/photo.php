...
{
    global $d, $idShop, $item, $type;

    $item = $d->rawQueryOne("select * from #_photo where id_shop = $idShop and act = ? and type = ? limit 0,1", array('photo_static', $type));
}

/* Save photo static */
function savePhotoStatic()
{
    global $d, $idShop, $func, $flash, $config, $type;

    /* Post dữ liệu */
    $row = $d->rawQueryOne("select id from #_photo where id_shop = $idShop and act = ? and type = ? limit 0,1", array('photo_static', $type));
    $message = '';
    $response = array();
        ...
        if ($d->update('photo', $data)) {
            ...
            if ($func->hasFile("file")) {
                ...
                if ($photo = $func->uploadImage("file", $config['photo']['photo_static'][$type]['img_type'], UPLOAD_PHOTO, $file_name)) {
                    $row = $d->rawQueryOne("select id, photo from #_photo where id_shop = $idShop and id = ? and act = ? and type = ? limit 0,1", array($id, 'photo_static', $type));
...
function viewPhotos()
{
    ...
    $sql = "select * from #_photo where id_shop = $idShop and type = ? and act <> ? order by numb,id desc $limit";
    $items = $d->rawQuery($sql, array($type, 'photo_static'));
    $sqlNum = "select count(*) as 'num' from #_photo where id_shop = $idShop and type = ? and act <> ? order by numb,id desc";
...
function editPhoto()
{
    global $d, $idShop, $func, $curPage, $item, $list_cat, $type;

    $id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

    if (empty($id)) $func->transfer("Không nhận được dữ liệu", "index.php?com=photo&act=man_photo&type=" . $type . "&p=" . $curPage, false);

    $item = $d->rawQueryOne("select * from #_photo where id_shop = $idShop and id = ? and act <> ? and type = ? limit 0,1", array($id, 'photo_static', $type));
    ...
    if ($id) {
        ...
        if ($d->update('photo', $data)) {
            ...
            if ($func->hasFile("file")) {
                ...
                if ($photo = $func->uploadImage("file", $config['photo']['man_photo'][$type]['img_type_photo'], UPLOAD_PHOTO, $file_name)) {
                    $row = $d->rawQueryOne("select id, photo from #_photo where id_shop = $idShop and id = ? and type = ? limit 0,1", array($id, $type));
        ...
        if ($numberPhoto && $dataMultiTemp) {
            ...
            for ($i = 0; $i < count($dataMultiTemp); $i++) {
                ...
                if ($success_photo && empty($response)) {
...
function deletePhoto()
{
    ...
    if ($id) {
        ...
        if (isset($row['id']) && $row['id'] > 0) {
            $func->deleteFile(UPLOAD_PHOTO . $row['photo']);
            $d->rawQuery("delete from #_photo where id_shop = $idShop and id = ? and type = ?", array($id, $type));
