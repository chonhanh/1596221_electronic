...
function viewSeoPage()
{
    ...
    $item = $d->rawQueryOne("select * from #_seopage where id_shop = $idShop and type = ? limit 0,1", array($type));
}

/* Save Seopage */
function saveSeoPage()
{
    global $d, $idShop, $func, $config, $com, $type;
        ...
        if ($d->update('seopage', $dataSeo)) {
            ...
            if ($func->hasFile("file")) {
                ...
                if ($photo = $func->uploadImage("file", $config['seopage']['img_type'], UPLOAD_SEOPAGE, $file_name)) {
                    $row = $d->rawQueryOne("select id, photo from #_seopage where id_shop = $idShop and type = ? limit 0,1", array($type));
