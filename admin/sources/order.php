...
function viewMans()
{
    ...
    if ($order_payment) $where .= " and A.order_payment=$order_payment";
    if ($order_date) {
    ...
    if (isset($_REQUEST['keyword'])) {
        ...
        $where .= " and (A.code LIKE '%$keyword%' or B.fullname LIKE '%$keyword%' or B.email LIKE '%$keyword%' or B.phone LIKE '%$keyword%')";
    }

    $perPage = 10;
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select A.*, B.fullname as fullname from #_order as A, #_order_info as B where A.id = B.id_order $where order by A.date_created desc $limit";
    $items = $d->rawQuery($sql);
    $sqlNum = "select count(*) as 'num' from #_order as A, #_order_info as B where A.id = B.id_order $where order by A.date_created desc";
...
function editMan()
{
    ...
    if (empty($id)) {
        ...
        $item = $d->rawQueryOne("select A.*, B.fullname as fullname, B.phone as phone, B.email as email, B.address as address from #_order as A, #_order_info as B where A.id = B.id_order and A.id = ? limit 0,1", array($id));
        ...
        if (empty($item)) {
            ...
            if (!empty($orderGroup)) {
                foreach ($orderGroup as $k_orderGroup => $v_orderGroup) {
                    ...
                    /* Get order detail */
                    $orderGroup[$k_orderGroup]['detail-lists'] = $d->rawQuery("select * from #_order_detail where id_order = ? and id_group = ?", array($id, $v_orderGroup['id']));
                }
            }
        }
    }
